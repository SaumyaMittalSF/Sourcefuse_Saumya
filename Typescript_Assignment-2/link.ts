...
// Decorator function for date time formatting
function DateTimeFormat(target: any, propertyKey: string) {
  const descriptor = Object.getOwnPropertyDescriptor(target, propertyKey);
  if (descriptor === undefined) return;

  const originalMethod = descriptor.value;
  descriptor.value = function (...args: any[]) {
    const formattedDate = this.formatDate(this[propertyKey]);
    console.log(`Formatted ${propertyKey}: ${formattedDate}`);
...
class UsertheTables<T extends User> {
  private users: T[] = [];
  ...
  private render(): void {
    ...
    // Edit a user row
    private editUser(row: HTMLTableRowElement, user: T): void {
      ...
      saveBtn.addEventListener('click', () => {
        ...
        user.role = roleInput.value as UserRole;
    ...
    // Delete a user row
    private deleteUser(row: HTMLTableRowElement, user: T): void {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (container) {
    const userTable = new UsertheTables<User>(container);
    userTable.init();
  } else {
